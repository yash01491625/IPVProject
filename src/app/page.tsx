...
const Blur = ({ blurType }: { blurType: 'gaussian' | 'median' }) => {
  ...
  const sendImageForProcessing = async (image: File) => {
    ...
    const res = await fetch(`http://localhost:5000/blur`, {
      method: "POST",
      body: formData,
    });
  ...
  const debouncedProcessing = useCallback(
    ...
    [kernelSize, sigma, blurType]
  );

  useEffect(() => {
    if (image) {
      debouncedProcessing(image);
    }
  }, [kernelSize, sigma, blurType]);

  return (
    <div className="flex flex-col items-center justify-center gap-4">
      <div className={`h-135 w-180 ${processedUrl ? "bg-card" : "bg-muted"} 
        ${processedUrl ? " border-muted border-2" : ""} rounded-md p-2 
        ${processedUrl ? "" : "animate-pulse"} mb-10`}>
        {processedUrl && (
          <Image
            src={processedUrl}
            alt="Blurred Preview"
      ...
      <div className="w-full max-w-md space-y-4">
        <div className="space-y-2">
          <label className="text-sm font-medium">Kernel Size (Odd numbers only)</label>
          <Slider
            value={[kernelSize]}
            min={1}
            max={31}
          ...
          <span className="text-xs text-muted-foreground">
            Current: {kernelSize}x{kernelSize}
          </span>
        </div>

        {blurType === 'gaussian' && (
          <div className="space-y-2">
            <label className="text-sm font-medium">Sigma (Standard Deviation)</label>
            <Slider
              value={[sigma]}
              min={0}
              max={15}
        ...
        <div className="w-[83vw] grid grid-cols-5 gap-4">
          <div className="w-full col-span-4 border-r-2 border-solid border-muted">
            <div className="mt-28 flex justify-center items-center">
              ...
              {isGaussian && <Blur blurType="gaussian" />}
              {isMedian && <Blur blurType="median" />}
            </div>
          </div>
